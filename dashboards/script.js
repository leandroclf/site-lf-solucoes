...
function renderAutonomyState(data) {
  ...
  const financeAutowire = data?.signals?.financeAutowire || {};
  ...
  const financeAutowireStatus = financeAutowire?.status || 'n/d';
  const financeAutowireCandidateCount = Number(financeAutowire?.candidateCount || 0);
  const financeAutowireSuggestedSource = financeAutowire?.recommendedSource?.path || null;
  ...
  signalMetaEl.textContent = `AUTO ativo: ${data?.stats?.activeAuto ?? 0} | prontos: ${data?.stats?.readyAuto ?? 0} | planejados: ${data?.stats?.plannedAuto ?? 0} | CI gate ISSUE-007: ${ciGate?.summary || 'n/d'} | ledger: ${ledger?.changeType || 'n/d'}${ledgerSuggestedSource ? ` | sugestão: ${ledgerSuggestedSource}` : ''} | autowire: ${financeAutowireStatus}${financeAutowireSuggestedSource ? ` | fonte: ${financeAutowireSuggestedSource}` : ''}${financeAutowireCandidateCount ? ` | candidatos: ${financeAutowireCandidateCount}` : ''}`;
  healthEl.textContent = `${Number(data?.autonomyScore || 0)}%`;
  healthMetaEl.textContent = `Bloqueios: ${data?.stats?.blockers ?? 0} | Fila: ${data?.stats?.actionQueue ?? 0} | Governança: ${governance?.newIssueIds?.length ?? 0} novas / ${governance?.packetNeeds?.length ?? 0} packets | Repos em alerta: ${Number(data?.stats?.yellowRepos || 0) + Number(data?.stats?.redRepos || 0)} | Ledger: ${ledger?.exists ? 'presente' : 'ausente'}${ledgerSuggestedSource ? ` | Sugestão: ${ledgerSuggestedSource}` : ''}${ledgerCandidateCount ? ` | Candidatos: ${ledgerCandidateCount}` : ''} | Autowire: ${financeAutowireStatus}${financeAutowireSuggestedSource ? ` | Fonte: ${financeAutowireSuggestedSource}` : ''}${financeAutowireCandidateCount ? ` | Candidatos: ${financeAutowireCandidateCount}` : ''}`;
